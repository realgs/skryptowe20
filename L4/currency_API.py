...
def get_daily_currency_markings(currency_code, date, max_hops = DEFAULT_MAX_HOPS):
    ...
    if response.status_code != 200:
        if response.status_code == 404:
            ...
            if max_hops > 0:
                return { date: get_daily_currency_markings(currency_code, previous_day, max_hops - 1)[previous_day] }
            else:
                return {date: -1.0}
        else:
        ...
        return { string_to_datetime(rate['effectiveDate']) : rate['mid'] }
    ...
    if markings:
        ...
        while(date_from < date_to):
            ...
            if date_from not in markings:
                ...
                markings[date_from] = markings[date_from - datetime.timedelta(1)]
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            for rate in response.json()['rates']:
                markings[string_to_datetime(rate['effectiveDate'])] = rate['mid']
