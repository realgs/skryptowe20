    ...
    <v-row class="mt-2" justify="center">
      <v-col cols="12" sm="6" md="3" offset-md="1">
        ...
        >
          ...
          >
            <v-spacer></v-spacer>
            <v-btn text color="primary" @click="start_picker = false">
              Cancel
            </v-btn>
            <v-btn
              text
      ...
      <v-col cols="12" sm="6" md="3">
        ...
        >
          ...
          >
            <v-spacer></v-spacer>
            <v-btn text color="primary" @click="end_picker = false">
              Cancel
            </v-btn>
            <v-btn
              text
      ...
      <v-col cols="12" sm="6" md="4" align="center">
        <v-btn class="ma-2" @click="getData" color="secondary">
          Generate
        </v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    responseToList: function () {
      let vals = [];
      for (var key in this.response) vals.push(this.response[key]);
      return vals;
    },
  },
  data: () => ({
  ...
  methods: {
    async requestForRange() {
      await axios
        .get(
          `http://127.0.0.1:5000/${this.url}/${this.date_start}/${this.date_end}`,
          { responseType: "json" }
        )
    ...
    getData() {
      ...
      } else alert("Start date must be smaller than end date");
    },
    renderRatesChart() {
      let dates = [];
      let rates = [];
