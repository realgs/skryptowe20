    ...
    <v-row class="mt-2">
        <v-col>
          ...
          <v-tab-item
            ...
            :key="tab.subpage"
          >
          <component class="mp-6" :is="tab.content" v-bind:url="tab.url"></component>
          </v-tab-item>
        </v-tabs-items>
        </v-col>
    </v-row>
...
<v-footer
  ...
  >
    <v-card
      flat
      tile
      class="primary white--text text-center flex"
    >
      <v-card-text>
        <v-btn
          v-for="icon in icons"
...
export default {
  ...
  data: () => ({
      ...
      tabs: [
        ...
        endpoints: [
          { desc: 'Get USD rates from  specified day', url: "http://127.0.0.1:5000/rates/{date}", color: "transparent"},
          { desc: 'Get USD rates from specified date range', url: "http://127.0.0.1:5000/rates/{start_date}/{end_date}", color: "grey lighten-2"},
