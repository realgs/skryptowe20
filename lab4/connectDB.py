...
def add_col_to_db(cursor):
    cursor.execute(f"ALTER TABLE {SALES_DATABASE} ADD CurrencyRate float(1)")
    cursor.execute(f"ALTER TABLE {SALES_DATABASE} ADD CurrencyType varchar(3)")
    cursor.commit()


def fill_col_usd_rate(cursor, date_from_string, date_to_string, currency_date_list, currency_mid_list, currency_type):
    cursor.execute(f"SELECT * FROM {SALES_DATABASE} WHERE OrderDate BETWEEN \'{date_from_string}\' AND \'"
                   f"{date_to_string}\'")
    select_list = []
    for row in cursor:
    ...
    for row in select_list:
        ...
        while row_date != currency_date_list[currency_index]:
            if row_date < currency_date_list[currency_index]:
                currency_index = 0
            currency_index += 1
        cursor.execute(f"UPDATE {SALES_DATABASE} SET CurrencyRate = {str(currency_mid_list[currency_index])}, "
                       f"CurrencyType = \'{str(currency_type)}\' WHERE SalesOrderID = {str(row[0])}")
        cursor.commit()


def get_total_sales_to_chart(cursor, date_from_string, date_to_string):
    cursor.execute(f"SELECT OrderDate, SUM(TotalDue), MAX(CurrencyRate) FROM {SALES_DATABASE} WHERE OrderDate BETWEEN "
