    ...
    for i in range(delta.days + 1):
        ...
        if day.strftime("%Y-%m-%d") in rates_dict:
            ...
            if day != start:
                ...
                k = 1
                resp = requests.get(const.URL + currency + '/' + (day - timedelta(days=k)).strftime("%Y-%m-%d") + '/')
                while resp.status_code != const.STATUS_OK:
                    k = k + 1
                    resp = requests.get(const.URL + currency + '/' + (day - timedelta(days=k)).strftime("%Y-%m-%d") + '/')
...
def get_currency_rates(start, end, currency):
    ...
    if start_year != end_year:
        resp = requests.get(const.URL + currency + '/' + start.strftime('%Y-%m-%d') +
                            '/' + str(start_year) + '-12-31/')
        if resp.status_code == const.STATUS_OK:
            rates_handle(rates_dict, resp)
            start_year = start_year + 1

    while start_year != end_year:
        resp = requests.get(const.URL + currency + '/' + str(start_year) + '-01-01/' + str(start_year) + '-12-31/')
        if resp.status_code == const.STATUS_OK:
            rates_handle(rates_dict, resp)
            start_year = start_year + 1

    if start != end:
        resp = requests.get(const.URL + currency + '/' + str(end_year) + '-01-01/' + end.strftime('%Y-%m-%d') + '/')
    else:
        resp = requests.get(const.URL + currency + '/' + end.strftime('%Y-%m-%d') + '/')
